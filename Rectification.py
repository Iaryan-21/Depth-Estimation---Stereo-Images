...
def rectify(img1, img2, F, pts1, pts2):
    ...
    _,H1,H2 = cv2.stereoRectifyUncalibrated(np.float32(pts1), np.float(pts2),F,imgSize=(w1,h1))
...
def draw_lines(lines_set, image,points):
    for line, pt in zip(lines_set, points):
        ...
        img = cv2.circle(image, (int(pt[0]), int(pt[1])),5,5,(0,255,0),2)
    return img
